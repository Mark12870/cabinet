...
public sealed class LensScenario(LensScenario.Installed installed)
    : IClassFixture<LensScenario.Installed>
{
    private const string Id = "lens";

    private const string Mix = "Wet Gain";

    private static readonly Dictionary<string, string> Bridges = new()
    {
        ["VST3"] = "Auburn Sounds Lens.vst3",
        ["LV2"] = "https://www.auburnsounds.com/products/Lens.html40733531#stereo",
    };
    ...
    public async Task OpensItsEditorAndSplitsSpectrum(string format)
    {
        Assert.True(
        ...
        Assert.InRange(audio.Peak, 0.3, 4);
    }
