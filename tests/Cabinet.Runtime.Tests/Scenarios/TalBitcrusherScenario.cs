...
public sealed class TalBitcrusherScenario(TalBitcrusherScenario.Installed installed)
    : IClassFixture<TalBitcrusherScenario.Installed>
{
    private const string Id = "tal-bitcrusher";

    private const string Mix = "Dry / Wet";

    private static readonly Dictionary<string, string> Bridges = new()
    {
        ["VST3"] = "TAL-Bitcrusher.vst3",
        ["VST2"] = "libTAL-Bitcrusher.so",
    };
    ...
    public async Task OpensItsEditorAndCrushes(string format)
    {
        Assert.True(
