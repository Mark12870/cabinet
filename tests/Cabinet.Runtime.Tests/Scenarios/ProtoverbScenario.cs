...
public sealed class ProtoverbScenario(ProtoverbScenario.Installed installed)
    : IClassFixture<ProtoverbScenario.Installed>
{
    private const string Id = "protoverb";
    ...
    public async Task OpensItsEditorAndReverberates(string format)
    {
        ...
        Assert.InRange(audio.Tail, 0.000012, 1);
        Assert.InRange(audio.Peak, 0.1, 1);
    }
