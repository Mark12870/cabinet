...
public sealed class RenegateScenario(RenegateScenario.Installed installed)
    : IClassFixture<RenegateScenario.Installed>
{
    private const string Id = "renegate";

    private const string Mix = "Mix";

    private static readonly Dictionary<string, string> Bridges = new()
    {
        ["VST3"] = "Auburn Sounds Renegate.vst3",
        ["LV2"] = "https://www.auburnsounds.com/products/Renegate.html40733431#stereo",
    };
    ...
    public async Task OpensItsEditorAndGates(string format)
    {
        Assert.True(
